...
files_to_log = [
    '/home/data1/lk/project/mol_tree/train_v2_qm9_comenet.py',
    '/home/data1/lk/project/mol_tree/tools.py',
    '/home/data1/lk/project/mol_tree/model_comenet.py',
    # '/home/data1/lk/project/mol_tree/dataset_v2.py'
    "/home/data1/lk/project/mol_tree/dataset_v2_test_comenet.py"
]
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
log_file = os.path.join(log_dir, f'{model_name}_qm9_{label_name}_training_log_{start_time}.log')
...
# 初始化模型和优化器
# modify
device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu") # cuda:0 cuda:1 cuda:2
# is_r2 = label_name == 'r2'
# model = GNNModel(device, is_r2=is_r2).to(device)  # Move model to GPU
model = GNNModel(device).to(device)  # Move model to GPU
# optimizer = optim.Adam(model.parameters(), lr=0.001)
if label_name in ['Cv', 'zpve', 'r2']:
...
def infer(model, dataloader, mode=None):
    ...
    with torch.no_grad():
        for iter, (graph1, graph2, target, paths, paths_labels, paths_labels_masks) in enumerate(dataloader):
            ...
            # elif label_name in ['zpve']:
            #     output[:, 1] = output[:, 1] * dataset.label_std + dataset.label_mean
            #     target[:, 1] = target[:, 1] * dataset.label_std + dataset.label_mean
            #     target[:, 2] = target[:, 2] * dataset.label_std + dataset.label_mean
            #     output[:, 0] = output[:, 0] * (target[:, 2] - dataset.label_mean) / target[:, 2]
            #     target[:, 0] = target[:, 0] * (target[:, 2] - dataset.label_mean) / target[:, 2]
...
# 更改学习率之后 要更改对应的train predict infer的metrics3和loss  和  dataset_v2_test_comenet.py中的target  crtl + f # modify