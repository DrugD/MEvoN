...
files_to_log = [
    '/home/data1/lk/project/mol_tree/train_v2_qm9_visnet.py',
...
if label_name in ['Cv', 'zpve']:
    optimizer = optim.AdamW(model.parameters(), lr=0.00002, weight_decay=1e-7)
...
elif label_name == 'lumo':
    optimizer = optim.AdamW(model.parameters(), lr=0.0001, weight_decay=1e-8)
else:
    optimizer = optim.AdamW(model.parameters(), lr=0.0002, weight_decay=1e-7)
...
def train(model, train_dataloader, val_dataloader, optimizer, scheduler, criterion, resume=False, epochs=1000):
        # 如果 resume 为 True，尝试加载检查点
    if resume:
        start_epoch, best_val_loss = load_checkpoint(model, optimizer, scheduler, checkpoint_dir='/home/data1/lk/project/mol_tree/checkpoints/visnet/r2', start_time='20250522_014210')
    else:
...
# 训练模型
train(model, train_dataloader, val_dataloader, optimizer, scheduler, criterion, resume=False, epochs=800)