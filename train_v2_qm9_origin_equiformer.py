...
from model_equiformer import Equiformer
# from dataset_v2 import MoleculeACDataset_v2
# from dataset_v2_test import MoleculeACDataset_v2
from dataset_v2_test_Ha2eV_equiformer import MoleculeACDataset_v2
...
log_file = os.path.join(log_dir, f'equiformer/only_qm9_{label_name}_training_log_{start_time}.log')
...
files_to_log = [
    '/home/data1/lk/project/mol_tree/train_v2_qm9_origin_equiformer.py',
    ...
    "/home/data1/lk/project/mol_tree/dataset_v2_test_Ha2eV_equiformer.py"
]
...
model = Equiformer(device=device).to(device)  # Move model to GPU
...
def train(model, train_dataloader, val_dataloader, optimizer, criterion, epochs=1000):
    ...
    for epoch in tqdm(range(epochs),desc=f'for {label_name}:'):
        ...
        for iter, (graph1, graph2, target, paths, paths_labels, paths_labels_masks) in enumerate(train_dataloader):
            ...
            log(f'Epoch [{epoch + 1}/{epochs}], {label_name} Iter [{iter + 1}/{len(train_dataloader)}], Loss: {loss.item():.6f}')

            
        avg_loss = total_loss / len(train_dataloader)
 

        log(f'Epoch [{epoch + 1}/{epochs}], {label_name} Loss: {avg_loss:.6f}, Loss1: {avg_loss:.6f}')
        
        # Validate
        val_loss = predict(model, val_dataloader)
        log(f'Epoch [{epoch + 1}/{epochs}], {label_name} Val Loss: {val_loss:.6f}, Val Loss1: {val_loss:.6f}')
