...
from model_gat import GNNModel
...
# Set up logging
files_to_log = [
    '/home/data1/lk/project/mol_tree/train_v2_qm9_origin_gat.py',
    '/home/data1/lk/project/mol_tree/tools.py',
    '/home/data1/lk/project/mol_tree/model_gat.py',
...
log_file = os.path.join(log_dir, f'only_{model_name}_qm9_{label_name}_training_log_{start_time}.log')
...
if label_name != 'Cv':
    optimizer = optim.AdamW(model.parameters(), lr=0.002, weight_decay=1e-4) # 0.0005
else:
    optimizer = optim.AdamW(model.parameters(), lr=0.002, weight_decay=1e-4) # 0.0005
...
def train(model, train_dataloader, val_dataloader, optimizer, criterion, epochs=1000):
    ...
    for epoch in tqdm(range(epochs),desc=f'for {label_name}:'):
        ...
        if val_loss < best_val_loss:
            ...
            pth_file = os.path.join(pth_dir, f'only_{model_name}_{label_name}_best_model_epoch_{start_time}.pth')
