...
# Set up logging
files_to_log = [
    '/home/data1/lk/project/mol_tree/train_v2_qm9_origin_comenet.py',
...
def extract_model_name(files_to_log):
    for file_path in files_to_log:
        if 'train_v2_qm9_origin_' in file_path:
            return file_path.split('train_v2_qm9_origin_')[-1].split('.py')[0]
...
log_file = os.path.join(log_dir, f'only_comenet_qm9_{label_name}_training_log_{start_time}.log')
...
def train(model, train_dataloader, val_dataloader, optimizer, criterion, epochs=1000):
    ...
    for epoch in tqdm(range(epochs),desc=f'for {label_name}:'):
        ...
        if val_loss < best_val_loss:
            ...
            pth_file = os.path.join(pth_dir, f'comenet_{label_name}_best_model_epoch_{start_time}.pth')
