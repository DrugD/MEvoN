...
from model_equiformer import Equiformer as GNNModel
...
log_file = os.path.join(log_dir, f'qm7/only_equiformer_training_log_{start_time}.log')
...
files_to_log = [
    '/home/data1/lk/project/mol_tree/train_v2_qm7_origin_equiformer.py',
    '/home/data1/lk/project/mol_tree/tools.py',
    # '/home/data1/lk/project/mol_tree/model.py',
    '/home/data1/lk/project/mol_tree/model_equiformer.py',
...
output_file = f"/home/data1/lk/project/mol_tree/graph/qm7/test_smiles.txt"
...
# import pdb;pdb.set_trace()
# 创建数据加载器
train_dataloader = DataLoader(train_dataset, batch_size=256, shuffle=True, num_workers=3)
val_dataloader = DataLoader(val_dataset, batch_size=256, shuffle=False, num_workers=3)

test_dataset = MoleculeACDataset_v2(input_file, dataset_file, test_file = output_file, mode='test')
test_dataloader = DataLoader(test_dataset, batch_size=256, shuffle=False, num_workers=3)

# 初始化模型和优化器
device = torch.device("cuda:7" if torch.cuda.is_available() else "cpu")
model = GNNModel(device=device).to(device)  # Move model to GPU
...
checkpoint_dir = os.path.join('checkpoints', 'only_equiformer', 'qm7')
...
def predict(model, dataloader, mode=None):
    ...
    with torch.no_grad():
        for graph1, graph2, target, paths, paths_labels, paths_labels_masks in dataloader:
            ...
            output = model(graph1, graph2, paths, paths_labels, paths_labels_masks)
...
def train(model, train_dataloader, val_dataloader, optimizer, criterion, epochs=10):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for iter, (graph1, graph2, target, paths, paths_labels, paths_labels_masks) in enumerate(train_dataloader):
            ...
            loss = criterion(output.squeeze().float(), target[:, 1].float())
            ...
            log(f'Epoch [{epoch + 1}/{epochs}], Iter [{iter + 1}/{len(train_dataloader)}], Loss: {loss.item():.6f}')

        avg_loss = total_loss / len(train_dataloader)

        log(f'Epoch [{epoch + 1}/{epochs}], Loss: {avg_loss:.6f}, Loss1: {avg_loss:.6f}')
        
        # Validate
        val_loss = predict(model, val_dataloader)
        log(f'Epoch [{epoch + 1}/{epochs}], Val Loss: {val_loss:.6f}, Val Loss1: {val_loss:.6f}')
...
def save_checkpoint(epoch, model, optimizer, scheduler, best_val_loss, checkpoint_dir, start_time):
    ...
    # 保存检查点
    pth_file = os.path.join(checkpoint_dir, f'only_gnn_model_epoch_{start_time}.pth')
    torch.save({
        'epoch': epoch,
